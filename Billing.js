...
function showBillingDashboard() {
  ...
  var custRows = Object.keys(byCustomer).sort().map(function(c) {
    var d = byCustomer[c];
    return '<tr><td>' + c + '</td><td class="r">$' + d.invoiced.toFixed(2) + '</td>' +
      '<td class="r">$' + d.paid.toFixed(2) + '</td><td class="r' + (d.outstanding > 0 ? ' ow' : '') + '">$' + d.outstanding.toFixed(2) + '</td></tr>';
  }).join('');
  
  var html = '<!DOCTYPE html><html><head><style>' +
    'body { font-family: Arial; padding: 15px; background: #f5f5f5; }' +
    '.dash { max-width: 700px; margin: 0 auto; }' +
    'h1 { color: #2E5339; font-size: 18px; border-bottom: 3px solid #4A7C59; padding-bottom: 8px; }' +
    ...
    '.btn-g { background: #4A7C59; color: #fff; }' +
    ...
    '</div>' +
    '<h3 style="color:#2E5339;margin-bottom:8px;">By Customer</h3>' +
  ...
  var output = HtmlService.createHtmlOutput(html).setWidth(750).setHeight(550);
