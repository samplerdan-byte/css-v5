...
function generateEndOfDayReport() {
  ...
  try {
    Logger.log('generateEndOfDayReport: starting...');
...
`;
  ...
  if (ui) ui.showModalDialog(htmlOutput, 'End of Day Report');
    ...
    Logger.log('generateEndOfDayReport: error: ' + e.message);
    if (typeof logError === 'function') logError('generateEndOfDayReport', 'Error generating report', { error: e.message, stack: e.stack });
...
function getFieldReportData() {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    Logger.log('getFieldReportData: error: ' + e.message);
    if (typeof logError === 'function') logError('getFieldReportData', 'Error gathering field report data', { error: e.message });
    return { warehouses: {} };
  }
}
...
function sendWarehouseFieldReport(warehouseName) {
  ...
  var emailHtml = '<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;margin:0;padding:20px;background:#f5f5f5;">' +
    ...
    try {
      ...
      result = { success: true, message: '✅ Report sent to: ' + recipients.join(', ') };
      Logger.log('sendWarehouseFieldReport: Sent to ' + warehouseName + ' (' + recipients.length + ' recipient(s))');
    } catch (emailErr) {
      result = { success: false, message: '❌ Email error: ' + emailErr.message };
      if (typeof logError === 'function') logError('sendWarehouseFieldReport', 'Failed to send field report email', { warehouse: warehouseName, recipients: recipients.join(', '), error: emailErr.message });
    }
    return result;
  } catch (e) {
    Logger.log('sendWarehouseFieldReport: error: ' + e.message);
    if (typeof logError === 'function') logError('sendWarehouseFieldReport', 'Error sending field report', { warehouse: warehouseName, error: e.message });
    return { success: false, message: '❌ Error: ' + e.message };
  }
}
