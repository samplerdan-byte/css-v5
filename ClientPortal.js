...
function generatePortalToken(customerName) {
  ...
  if (!sheet) {
    ...
    if (!sheet) {
      SpreadsheetApp.getUi().alert('Error: Could not create Portal Access sheet.');
      return;
    }
  }
  ...
  if (sheet.getLastRow() >= 2) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (String(data[i][0]).trim().toLowerCase() === customerName.toLowerCase()) {
        ...
        // Update existing row — token (col 3) and created date (col 5)
        sheet.getRange(i + 2, 3).setValue(token);
        sheet.getRange(i + 2, 5).setValue(new Date());
        Logger.log('Regenerated token for existing customer: ' + customerName);
  ...
  Logger.log('New portal token created for: ' + customerName);
  _showPortalLink(customerName, token);
}
...
function _validatePortalToken(token) {
  ...
  // Sanitize: tokens should only contain alphanumeric characters
  token = token.trim();
  if (!/^[A-Za-z0-9]+$/.test(token) || token.length > 50) return null;
...
function getPortalData(token) {
  ...
  try {
    ...
    Logger.log('getPortalData: returning ' + result.orders.length + ' orders for ' + auth.customerName);
    return result;
  } catch (e) {
    Logger.log('Error in getPortalData: ' + e.message);
    return { success: false, message: 'An error occurred loading portal data. Please try again.' };
  }
}
